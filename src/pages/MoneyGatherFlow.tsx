        ...
        {tab === 'play' && buildLines.length > 0 && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-8">
            ...
            <div className="flex items-center gap-6">
              <button aria-label="Previous line" onClick={() => setPlayIndex(p => Math.max(0, p - 1))}><ChevronLeft size={22} className="text-muted-foreground hover:text-foreground" /></button>
              <button aria-label={playing ? "Pause" : "Play"} onClick={togglePlay} className="w-14 h-14 rounded-full bg-soul-gold/20 flex items-center justify-center text-soul-gold hover:bg-soul-gold/30 transition-colors">
                {playing ? <Pause size={22} /> : <Play size={22} />}
              </button>
              <button aria-label="Next line" onClick={() => setPlayIndex(p => Math.min(buildLines.length - 1, p + 1))}><ChevronRight size={22} className="text-muted-foreground hover:text-foreground" /></button>
            </div>
          </div>
        )}
      </div>
