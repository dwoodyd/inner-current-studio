    ...
    <div className="relative">
      ...
      >
        ...
        <div className="space-y-3">
          {items.map(({ icon: Icon, title, description, to, accent }, i) => {
            ...
            return (
              <motion.button
                ...
                variants={fadeUp}
                onClick={() => navigate(to)}
                className={`soul-glass-elevated w-full text-left flex items-start gap-4 rounded-2xl p-5 transition-all hover:scale-[1.01] active:scale-[0.98] ${a.glow}`}
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.98 }}
              >
                <div
                  className="mt-0.5 h-10 w-10 rounded-xl flex items-center justify-center shrink-0"
