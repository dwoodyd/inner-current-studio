          ...
          {playLines.length === 0 ? (
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-3">
                <button aria-label="Previous line" onClick={() => setIdx(i => (i - 1 + playLines.length) % playLines.length)} className="soul-card p-3 rounded-full"><ChevronLeft size={20} className="text-muted-foreground" /></button>
                <button aria-label={playing ? "Pause" : "Play"} onClick={togglePlay} className="soul-glass-elevated p-5 rounded-full">
                  {playing ? <Pause size={22} className={domain.accentClass} /> : <Play size={22} className={domain.accentClass} />}
                </button>
                <button aria-label="Next line" onClick={() => setIdx(i => (i + 1) % playLines.length)} className="soul-card p-3 rounded-full"><ChevronRight size={20} className="text-muted-foreground" /></button>
              </div>
            </>
          )}
        </div>
